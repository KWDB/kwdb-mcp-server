...
// TestRegisterToolsWithConfig verifies that RegisterToolsWithConfig registers
// the canonical tool set in a stable order, and that the removed
// query-metrics-history tool no longer appears in the registry.
func TestRegisterToolsWithConfig(t *testing.T) {
	s := mcpserver.NewMCPServer("test", "1.0", mcpserver.WithToolCapabilities(true))
	RegisterToolsWithConfig(s, Config{})

	registered := s.ListTools()

	wantTools := []string{"read-query", "write-query", "query-metrics", "query-slow-sql"}
	for _, name := range wantTools {
		if _, ok := registered[name]; !ok {
			t.Errorf("expected tool %q to be registered; registered=%v", name, toolNames(registered))
		}
	}

	if _, ok := registered["query-metrics-history"]; ok {
		t.Errorf("query-metrics-history must not be registered; registered=%v", toolNames(registered))
	}

	// Stable registration order: read-query, write-query, query-metrics, query-slow-sql.
	gotOrder := make([]string, 0, len(registered))
	for _, name := range wantTools {
		if _, ok := registered[name]; ok {
			gotOrder = append(gotOrder, name)
		}
	}
	if len(gotOrder) != len(wantTools) {
		t.Fatalf("registered tool count mismatch: got=%v want=%v", gotOrder, wantTools)
	}
	for i, name := range wantTools {
		if gotOrder[i] != name {
			t.Errorf("registration order mismatch at index %d: got=%q want=%q (full=%v)", i, gotOrder[i], name, gotOrder)
		}
	}
}

func toolNames(registered map[string]*mcpserver.ServerTool) []string {
